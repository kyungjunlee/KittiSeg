...
def retrain_model():
  ...
  now = datetime.now()
  model_id = now.strftime("%Y%m%d%H%M%S")
  try:
    FLAGS = FlagClass()
    FLAGS.bottleneck_dir = "/tmp/bottlenecks"
    FLAGS.how_many_training_steps = 400
    FLAGS.summaries_dir = "classifier/training_summaries/inception_v3/%s" % (model_id)
    FLAGS.output_graph = "models/classifier_graph_%s.pb" % (model_id)
    FLAGS.output_labels = "models/classifier_labels_%s.txt" % (model_id)
    FLAGS.image_dir = "testing/"
    ...
    FLAGS.saved_model_dir = 'models/%s/' %(model_id)
