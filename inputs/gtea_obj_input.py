...
def maybe_download_and_extract(hypes):
    ...
    # GTEA DATA
    logging.info("Preparing GTEA obj data.")
...
def _make_data_gen(hypes, phase, data_dir):
    ...
    obj_color = np.array(hypes['data']['obj_color'])
    ...
    for image, gt_image in data:
        ...
        gt_obj = np.all(gt_image == obj_color, axis=2)
        ...
        """
        # probability of object of interest at each pixel
        shape = image.shape
        gt_image = np.max(gt_image / 255, axis = 2)
        gt_image = gt_image.reshape(shape[0], shape[1], 1)
...
def create_queues(hypes, phase):
    ...
    arch = hypes['arch']
    """
    Output should be probability, so change tf.int32 to tf.float32
    """
...
def main():
    """main."""
    with open('../hypes/GTEA_obj.json', 'r') as f:
        hypes = json.load(f)
    ...
    image_batch, prob_batch = inputs(hypes, q, 'train')
    ...
    with tf.Session() as sess:
        ...
        for i in itertools.count():
            ...
            scp.misc.imshow(gt[0] > 0)
            # gt_bg = gt[0, :, :, 0]
            # gt_obj = gt[0, :, :, 1]
            # scp.misc.imshow(gt_bg)
            # scp.misc.imshow(gt_obj)
