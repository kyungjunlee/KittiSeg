...
def _make_data_gen(hypes, phase, data_dir):
    ...
    for image, gt_image in data:
        ...
        gt_hand1 = np.all(gt_image == hand_color1, axis=2)
        gt_hand2 = np.all(gt_image == hand_color2, axis=2)
        gt_hand3 = np.all(gt_image == hand_color3, axis=2)

        assert(gt_hand1.shape == gt_bg.shape)
        shape = gt_bg.shape
        ...
        # two hands concatenation
        gt_hand = gt_hand1 + gt_hand2 + gt_hand3
        # gt_image concatenation
        gt_image = np.concatenate((gt_bg, gt_hand), axis=2)
...
def main():
    """main."""
    with open('../hypes/gtea.json', 'r') as f:
        hypes = json.load(f)
