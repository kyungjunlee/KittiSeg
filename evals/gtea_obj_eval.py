...
def eval_image(hypes, gt_image, cnn_image):
    """
    As of now, gt_image should contain its raw, original RGB value in each pixel
    """

    thresh = np.array(range(0, 256))/255.0

    # obj_color = np.array(hypes['data']['obj_color']) + 1
    background_color = np.array(hypes['data']['background_color'])
    # gt_obj = np.all(gt_image != background_color, axis=2) # TODO: may not exist
    ...
    TODO: for now, use max to extract the highest probability among the RGB channels
    """
    gt_obj = np.max(gt_image / 255, axis = 2)
    gt_obj = np.array(gt_obj > 0)
    valid_gt = gt_obj + gt_bg

    # cnn_image = np.all(cnn_image > 0)

    FN, FP, posNum, negNum = evalExp(gt_obj, cnn_image,
...
def evaluate(hypes, sess, image_pl, inf_out):
    ...
    # euclidean = inf_out['euclidean']
    # logits = inf_out['logits']
    """
    TODO:
    """
    sigmoid = inf_out['sigmoid']
    ...
    for phase in ['train', 'val']:
        ...
        with open(data_file) as file:
            for i, datum in enumerate(file):
                    ...
                    output = sess.run([sigmoid], feed_dict=feed_dict)
                    ...
                    if phase == 'val':
                        ...
                        hard = output_im > 0.5
                        red_image = utils.fast_overlay(image, hard, color = [0, 100, 100, 255])
        ...
        if phase == 'val':
            ...
            for i in xrange(10):
                sess.run([sigmoid], feed_dict=feed_dict)
